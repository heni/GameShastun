...
func calc_game_table(strategy StrategyOptions, skips_table *[]PrecalcSkip) []float64 {
	...
	for _, pos := range calculation_queue[1:] {
		...
		slices.SortFunc(next_moves, func(a, b TimedMove) int {
			return int(math.Copysign(1, a.time_to_win-b.time_to_win))
		})

		sum_tm, skip_tm := 0.0, 0.0
		extra_skips := 0
		...
		if strategy.do_optimal_skip {
			...
			for i := len(next_moves) - 1; i > 0; i-- {
				skip_tm += next_moves[i].time_to_win
				extra_skips++
				...
				moves_to_skip = append(moves_to_skip, next_moves[i].move)
				speedup_time += best_time - cand_time
				best_time = cand_time
			}
			if len(moves_to_skip) > 0 && skips_table != nil {
